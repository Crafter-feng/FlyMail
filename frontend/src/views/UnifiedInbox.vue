<template>
  ...
  :key="att.part_number"
  @click="openAttachmentMenu(att, $event)"
  >
  <div class="att-icon">
...
import { getInitial, getAvatarColor, formatDate, formatDetailDate, formatFileSize, extractEmails, extractName, downloadAttachment as downloadAttachmentFile, saveAttachmentToNas, formatAddressList } from '../utils/mail-helpers';
...
.mail-view { display: flex; flex-direction: column; flex: 1; height: 100%; min-height: 0; overflow: hidden; }
...
/* 附件下载菜单 */
.att-menu-overlay { position: fixed; inset: 0; z-index: 1500; }
.att-menu {
  position: fixed; min-width: 200px; background: var(--bg-card);
