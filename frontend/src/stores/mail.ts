...
export const useMailStore = defineStore('mail', () => {
  ...
  *
  * 优先使用 folder_stats（IMAP STATUS）的值。
  * QQ邮箱的 select(readonly=True) + search(UNSEEN) 会返回0（QQ IMAP的bug），
  * 所以当新值为 0 且已有非零值时，保留旧值。
  */
  ...
  // QQ 邮箱 SELECT+SEARCH 返回 0 时保留已有值，其他情况始终更新
  const shouldUpdateUnread = unreadTotal > 0 || !existing || existing.unread_count === 0;
  folderCounts.value = {
  ...folderCounts.value,
  [folderName]: {
  unread_count: shouldUpdateUnread ? unreadTotal : existing!.unread_count,
  total_count: total,
  },
  };
  sessionStorage.setItem('flymail_folder_counts', JSON.stringify(folderCounts.value));
  ...
  */
  ...
  async function saveSortOrder(orders: { id: string; sort_order: number }[]) {
  await api.put('/accounts/sort-order', orders);
  // 更新本地 accounts 的 sort_order
