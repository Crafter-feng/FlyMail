...
async def _verify_folder_stats_background(account: Account, folder: str):
    ...
    """
    # 原生 IDLE provider 的 INBOX 已由后台长连接监听，跳过统计校验避免创建额外连接。
    if account.provider in sync_service.IDLE_PROVIDERS and folder.upper() == "INBOX":
        return
    ...
    try:
        ...
        counts = None
        if account.provider in sync_service.IDLE_PROVIDERS:
        ...
        elif account.provider in sync_service.POLL_PROVIDERS:
        ...
        # 其他 provider（如 netease/sina/custom）：创建新连接，保留旧方式
        credentials = await _ensure_gmail_token(account)
        receiver = ProviderFactory.get_receiver(account.provider)
        await receiver.connect(credentials)
        try:
...
async def refresh_messages(
    ...
    当后台监听检测到新邮件但缓存同步失败时，前端可调用此接口强制刷新。
    也可以用于用户手动下拉刷新。
    """
    user_uid = await get_uid(request)
    accounts = await get_accounts(user_uid)
...
async def get_message_detail(
    ...
    try:
        ...
        existing_is_read = await get_cached_is_read(account.id, int(_extract_uid(message_id)), folder)
        ...
        try:
            ...
            for msg_id in message_ids:
                try:
                    ...
                    cm = CachedMessage(
                        id=make_cached_message_id(account.id, folder, uid_num),
                        account_id=account.id,
                        user_uid=user_uid,
                        uid=uid_num,
                        folder=folder,
