...
class ProxyIMAP4_SSL(aioimaplib.IMAP4_SSL):
    """aioimaplib IMAP4_SSL client that connects through an HTTP CONNECT proxy."""
...
class IdleConnection:
    ...
    def __init__(self, account_id: str, host: str, port: int, email: str,
        ...
        # aioimaplib.idle_start() 返回的 IMAP4.idle() future。
        # 正常路径必须通过 DONE 让它自然完成，不能提前 cancel，否则会破坏 IDLE 状态机。
        self._idle_task: Optional[asyncio.Future] = None
            ...
            try:
                ...
                if idle_task and not idle_task.done():
                    try:
                        ...
                        logger.debug("等待 IDLE 任务完成超时，取消残留任务")
                        idle_task.cancel()
                        await asyncio.gather(idle_task, return_exceptions=True)
                    except Exception as e:
                        logger.debug("等待 IDLE 任务完成失败: %s", e)
                elif idle_task:
                    try:
                        idle_task.result()
                    except asyncio.CancelledError:
                        ...
                        logger.debug("IDLE 任务已结束，读取结果时忽略异常: %s", e)
    ...
    def _decode_idle_line(self, line) -> str:
        if isinstance(line, bytes):
            return line.decode('utf-8', errors='ignore')
        return str(line)

    def _drain_idle_queue(self) -> list:
        """取出 aioimaplib idle_queue 中已经 flush 的事件，避免 DONE 边界事件滞留到下一轮。"""
        if not self.client:
            return []
        protocol = getattr(self.client, "protocol", None)
        queue = getattr(protocol, "idle_queue", None)
        if queue is None:
            return []
    ...
    def _classify_idle_lines(self, folder: str, lines) -> str:
        """将 IMAP IDLE 推送行归类为业务事件。"""
        if not lines:
            return "timeout"

        for line in lines:
            line_str = self._decode_idle_line(line)
            upper_line = line_str.upper()
            if "STOP_WAIT_SERVER_PUSH" in upper_line:
                continue
            if "EXISTS" in upper_line:
                logger.info("IDLE 检测到新邮件: folder=%s, response=%s", folder, line_str)
                return "new_mail"
            if "EXPUNGE" in upper_line:
                logger.info("IDLE 检测到邮件删除: folder=%s, response=%s", folder, line_str)
                return "expunge"
            if "FETCH" in upper_line and "FLAGS" in upper_line:
                return "fetch"
        return "timeout"
    ...
    async def idle_wait(self, folder: str, timeout: int = IDLE_WAIT_TIMEOUT) -> str:
        ...
        queued_event = self._classify_idle_lines(folder, self._drain_idle_queue())
        if queued_event != "timeout":
            return queued_event
        ...
        try:
            ...
            self.connected = False
            raise ConnectionError(f"IDLE 启动失败: {e}") from e
        ...
        try:
            ...
            event = self._classify_idle_lines(folder, response)
        except asyncio.TimeoutError:
            # IDLE 超时是正常行为，不是错误
            event = "timeout"
        finally:
            ...
            if idle_task:
                try:
                    if not idle_task.done():
                        self.client.idle_done()
                        idle_response = await asyncio.wait_for(idle_task, timeout=5.0)
                    else:
                        idle_response = idle_task.result()

                    done_event = self._classify_idle_lines(folder, getattr(idle_response, "lines", None))
                    if event == "timeout" and done_event != "timeout":
                        event = done_event
                except asyncio.CancelledError:
                    raise
                except asyncio.TimeoutError:
                    self.connected = False
                    cleanup_error = ConnectionError("IDLE DONE 后等待服务端确认超时")
                    idle_task.cancel()
                    await asyncio.gather(idle_task, return_exceptions=True)
                except Exception as e:
                    self.connected = False
            ...
            queued_event = self._classify_idle_lines(folder, self._drain_idle_queue())
            if event == "timeout" and queued_event != "timeout":
                event = queued_event
            self._idle_task = None

        if cleanup_error and event == "timeout":
            raise ConnectionError(f"IDLE 连接已断开: {cleanup_error}") from cleanup_error
        return event
...
class PollConnection:
    ...
    async def poll_wait(self, folder: str, interval: int = POLL_INTERVAL, timeout: int = POLL_TIMEOUT) -> str:
        ...
        last_count = self._folder_counts.get(folder)
        if last_count is None:
            last_count = await self._get_folder_count(folder)
            if last_count < 0:
        ...
        while elapsed < timeout:
            ...
            try:
                ...
                if current_count > last_count:
                    logger.info("轮询检测到新邮件: folder=%s, %d -> %d", folder, last_count, current_count)
                    self._folder_counts[folder] = current_count
                    return "new_mail"
                if current_count < last_count:
                    logger.info("轮询检测到邮件减少: folder=%s, %d -> %d", folder, last_count, current_count)
                    self._folder_counts[folder] = current_count
                    return "expunge"
                self._folder_counts[folder] = current_count
