...
# 开发环境默认为当前目录下的 data/
_data_dir = os.environ.get("FLYMAIL_DATA_DIR", os.path.join(os.path.dirname(__file__), "..", "data"))
_jobstore_db_path = os.path.join(_data_dir, "jobs.sqlite")
_jobstore_url = f"sqlite:///{_jobstore_db_path}"
    ...
    try:
        ...
        try:
            await sender.send_message(
                ...
                cc=cc or None,
                bcc=bcc or None,
                attachments=safe_attachment_paths or None,
...
def start_scheduler():
    ...
    失败时仅记录错误日志，主服务（邮件收发、实时监听）仍可正常工作。
    """
    s = _get_scheduler()
    if not s.running:
        try:
            s.start()
            logger.info("定时发送调度器已启动，任务存储: %s", _jobstore_db_path)
        except Exception as e:
