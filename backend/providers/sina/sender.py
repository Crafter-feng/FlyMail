...
from .config import (
    SMTP_SINA_COM_HOST, SMTP_SINA_COM_PORT,
    SMTP_SINA_CN_HOST, SMTP_SINA_CN_PORT,
    SMTP_VIP_SINA_COM_HOST, SMTP_VIP_SINA_COM_PORT,
    SMTP_VIP_SINA_CN_HOST, SMTP_VIP_SINA_CN_PORT,
)
from utils.logger import get_logger

logger = get_logger("sina.sender")


class SinaSender(MailSender):
    """新浪邮箱 SMTP 发送器（支持 sina.com / sina.cn / 2008.sina.com / vip.sina.com / vip.sina.cn）

    服务器选择逻辑与 SinaReceiver._get_imap_host 对称，按邮箱后缀区分 4 套 SMTP 服务器。
    """
    ...
    def _get_smtp_host(self, email_addr: str) -> str:
        """根据邮箱后缀返回对应的 SMTP 服务器地址

        新浪邮箱按后缀区分 4 套独立 SMTP 服务器：
        - sina.cn → smtp.sina.cn
        - vip.sina.com → smtp.vip.sina.com
        - vip.sina.cn → smtp.vip.sina.cn
        - sina.com / 2008.sina.com 及其他 → smtp.sina.com（默认）
        """
        suffix = email_addr.split("@")[-1].lower() if "@" in email_addr else ""
        host_map = {
            "sina.cn": SMTP_SINA_CN_HOST,
            "vip.sina.com": SMTP_VIP_SINA_COM_HOST,
            "vip.sina.cn": SMTP_VIP_SINA_CN_HOST,
        }
        # sina.com / 2008.sina.com 及其他未知后缀默认走 smtp.sina.com
        return host_map.get(suffix, SMTP_SINA_COM_HOST)

    def _get_smtp_port(self, email_addr: str) -> int:
        """根据邮箱后缀返回对应的 SMTP 端口（所有新浪邮箱统一 465）"""
        return 465

    async def connect(self, credentials: Credentials) -> None:
        """连接到新浪邮箱 SMTP 服务器"""
        self.email_addr = credentials.extra.get("email", "")
        auth_code = credentials.access_token  # 新浪邮箱使用授权码

        try:
            self.conn = await asyncio.to_thread(self._connect_smtp, self.email_addr, auth_code)
        except Exception as e:
            self.conn = None
            raise Exception(f"新浪邮箱SMTP连接失败: {str(e)}")
