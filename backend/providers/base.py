...
class MailReceiver(ABC):
    ...
    async def idle_wait(self, folder: str = "INBOX", timeout_seconds: int = 1740) -> bool:
        ...
        - IDLE provider（Gmail/Outlook）：由 idle_manager 的 IdleConnection 管理
        - Poll provider（QQ/iCloud）：由 poll_manager 的 PollConnection 管理
        - NOOP provider（网易/新浪/自定义邮箱）：覆盖此方法做单连接 STATUS/NOOP 轮询
        """
