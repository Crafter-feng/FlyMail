...
from providers.qq.config import SMTP_HOST, SMTP_PORT, EXMAIL_SMTP_HOST, EXMAIL_SMTP_PORT
...
class QQSender(MailSender):
    """QQ邮箱SMTP发送器（支持QQ邮箱和腾讯企业邮箱）"""

    SMTP_HOST = SMTP_HOST
    SMTP_PORT = SMTP_PORT
    EXMAIL_SMTP_HOST = EXMAIL_SMTP_HOST
    EXMAIL_SMTP_PORT = EXMAIL_SMTP_PORT
    ...
    def _get_smtp_host(self) -> str:
        """根据邮箱类型返回 SMTP 服务器地址"""
        return self.EXMAIL_SMTP_HOST if self._is_exmail else self.SMTP_HOST

    def _get_smtp_port(self) -> int:
        """根据邮箱类型返回 SMTP 端口"""
        return self.EXMAIL_SMTP_PORT if self._is_exmail else self.SMTP_PORT

    async def connect(self, credentials: Credentials) -> None:
        """连接到QQ邮箱/腾讯企业邮箱SMTP服务器，支持重试"""
