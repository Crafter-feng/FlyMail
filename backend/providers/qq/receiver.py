...
from providers.qq.config import IMAP_HOST, IMAP_PORT, EXMAIL_IMAP_HOST, EXMAIL_IMAP_PORT
...
class QQReceiver(BaseIMAPReceiver):
    """QQ邮箱IMAP接收器（支持QQ邮箱和腾讯企业邮箱）

    根据 credentials.extra 中的 is_exmail 标志自动选择服务器：
    ...
    IMAP_HOST = IMAP_HOST
    IMAP_PORT = IMAP_PORT
    EXMAIL_IMAP_HOST = EXMAIL_IMAP_HOST
    EXMAIL_IMAP_PORT = EXMAIL_IMAP_PORT
    ...
    def __init__(self):
        ...
        self._is_exmail: bool = False  # 是否为腾讯企业邮箱
    ...
    def _get_imap_host(self) -> str:
        """根据邮箱类型返回 IMAP 服务器地址"""
        return self.EXMAIL_IMAP_HOST if self._is_exmail else self.IMAP_HOST

    def _get_imap_port(self) -> int:
        """根据邮箱类型返回 IMAP 端口"""
        return self.EXMAIL_IMAP_PORT if self._is_exmail else self.IMAP_PORT
